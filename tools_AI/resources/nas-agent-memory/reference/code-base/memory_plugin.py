...
class MemoryPlugin(BasePlugin):
    ...
    async def _on_task_completed(self, event):
        ...
        - errors_encountered = True (troubleshooting)
        - duration_seconds > 300 (más de 5 min)
        """
        ...
        summary = event.data.get("summary", "Tarea compleja completada")
        title = summary[:60]
        session_content = (
            f"# Sesión: {title}\n"
            f"> Tools: {tool_calls} | "
        ...
        # Si hubo troubleshooting, sugerir skill
        # (en implementación real, pedirle al modelo que genere el skill)
        if had_errors:
            pass  # TODO: invoke model to generate skill

    async def _on_user_correction(self, event):
        """Persiste correcciones del usuario en USER.md."""
        from agent.core.memory import MemoryManager

        correction = event.data.get("correction", "")
        # Extraer key/value de la corrección
        # (simplificado — en implementación real, usar el modelo)
        MemoryManager.update_user_model(
            key="correccion_reciente",
            value=correction[:100],
        )

    # ── Capa C: curación ───────────────────────────────────────

    async def _curate_memory(self):
        """Curación diaria: limpiar, consolidar, verificar."""
        from agent.core.memory import MemoryManager
        ...
        # 3. Verificar skills (TODO: pedir al modelo que valide)
        # 4. Consolidar duplicados (TODO: pedir al modelo)
        # Estas tareas requieren una llamada al LLM —
        # se implementarán cuando el agente tenga acceso al modelo
        # dentro del contexto del schedule.
